...
def classify_doc(text, aspect):
    ...
    for j,d in enumerate(text.split('</s>')):
        doc = clean_str(d)
        for w in doc.split(' '):
            if w in word2topic and w in topics[aspect]:
                tmp_list.append(w)
                if w not in doc_num:
                    doc_num[w] = []
                doc_num[w].append(j)
    if len(tmp_list) > 0:
        ...
        topic_words[aspect] = [w for w in tmp_list if len(set(doc_num[w])) > 1]
        if len(topic_words[aspect]) > 0 and (topic_name[aspect] not in topic_words[aspect]):
            tmp = topic_words[aspect]
            topic_words[aspect] = [topic_name[aspect]]
            topic_words[aspect].extend(tmp)
...
corpus_file = '/shared/data2/jiaxinh3/summarization/BERT-Transformer-for-Summarization/data/processed_data/yelp/aspect_test.txt'
output_file = '/shared/data2/jiaxinh3/summarization/BERT-Transformer-for-Summarization/data/processed_data/yelp/aspect_test_extract.txt'
corpus = []
class_num_count = {}
doc_length_total = 0
max_doc_length = 0
...
with open(output_file,'w') as fout:
    with open(corpus_file) as f:
        ...
        for line in tqdm(f, total=200):
            # print(str(index+1))
            aspect = topic_name.index(line.split('\t')[1])
            docs = line.split('\t')[2]
            doc_class = classify_doc(docs, aspect)
